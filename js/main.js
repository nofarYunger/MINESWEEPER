...
function WhichButton(event, elCell) {
    ...
    //save current board to temp 
    var tempBoard = duplicate();
    var result
    switch (event.button) {
        case 0:
            // left click
            result = cellClicked(elCell);
            break;
        case 2:
            // right click
            result = cellMarked(elCell);
            break;
        default: return;
    }
    // push temp board to stack

    if (checkGameOver()) gameWon();

    if (result) {
        gPlayedMoves.unshift(tempBoard)
        gGame.turnCount++
    }
...
function cellClicked(elCell) {
    ...
    if (cellClasses.contains('show')) return false;
    if (cellClasses.contains('marked')) return false;
...
function cellMarked(elCell) {
    ...
    if (elCell.classList.contains('show')) return false;
